...
func TestPickDevice(t *testing.T) {
	devs := []govee.Device{
		{IP: "192.168.68.64", Device: "dev-a", SKU: "H60A1"},
		{IP: "192.168.68.70", Device: "dev-b", SKU: "H6001"},
	}
	d, ok := pickDevice(devs, "", "dev-b", "")
	if !ok || d.IP != "192.168.68.70" {
		t.Fatalf("by device id: %+v %v", d, ok)
	}
	d, ok = pickDevice(devs, "192.168.68.64", "", "")
	if !ok || d.Device != "dev-a" {
		t.Fatalf("by ip: %+v %v", d, ok)
	}
	d, ok = pickDevice(devs, "", "", "H60A1")
	if !ok || d.IP != "192.168.68.64" {
